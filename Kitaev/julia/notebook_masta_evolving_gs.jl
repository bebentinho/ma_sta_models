...
# ╔═╡ ed236c42-6511-43a9-8ae8-6c3a88e6bdb4
using Markdown

# ╔═╡ cfe73149-5d2b-4da6-95d8-be268106de44
using InteractiveUtils

# ╔═╡ ae3c78fd-5ffa-492a-8168-d09e6e8ae8fb
using LinearAlgebra

# ╔═╡ 5bfc2327-b929-40d3-ac01-f7d1a2477897
using QuantumOptics

# ╔═╡ 12a8a807-8054-4813-a03d-c004e8b8bbf3
using PyPlot

# ╔═╡ 4230b63f-53ee-4e85-a359-0edca7954e58
omega = 1 # Energy hopping

# ╔═╡ c8feee5f-c171-4815-b990-e16f46f4f674
g_0 = 0

# ╔═╡ 5b457aca-b375-4308-b47b-063f606c2b0a
npart = Int(14)

# ╔═╡ 18963f9c-5fb5-49f8-b4e5-d460faf73ba0
tau = Int(20) # total time evolution

# ╔═╡ 82e2e0af-2aaa-45fc-bce9-0943827fa272
delta = 1

# ╔═╡ bf668183-6e15-450a-b825-580f4a4fc1b3
g_f = -3*omega

# ╔═╡ 43afd713-dc9d-45ba-b435-fec4ffd358b5
alpha_gvars = (64 * abs(delta)^2 * (sin(pi/npart))^2)^(-1)

# ╔═╡ 822476f7-22ae-4473-96e0-97d3b68b5de8
beta_gvars = -2 * omega * cos(pi/npart)

# ╔═╡ fed7564a-14d0-41f3-b99f-dc85642b7bfa
gamma_gvars = 2 * abs(delta) * sin(pi/npart)

# ╔═╡ d596e752-9675-433b-87b7-4143f4ba7365
gvars = Dict("alpha" => alpha_gvars, "beta" => beta_gvars, "gamma" => gamma_gvars, "tau" => tau)

# ╔═╡ bd30a922-4c65-4f13-9bc3-c538e576c74d
gvars_lin = Dict("gf" => g_f, "tau" => tau)

# Formatting parameters

# ╔═╡ cb65c979-9822-40cc-8c34-e12c189f8d58
amt_times = 200 # How many points we calculate

# ╔═╡ a5158774-4dba-4a33-a95a-0fd866e49f8b
ts = LinRange(0, tau, amt_times)

# ╔═╡ 2309ba77-1d31-4d8b-b318-91fa7f594999
...
# ╔═╡ a634d080-46b7-405a-9b6a-a121fe405da5
function g_lin_t(t, gvars)
    # Linear ramp solution
    g_0 + (gvars["gf"] - g_0) * t / gvars["tau"]
end

# ╔═╡ fef5ad94-b3a7-4fbb-9f37-e3d1222f3f9f
id = identityoperator(SpinBasis(1/2))

# ╔═╡ 6e1cfbe7-7fe6-478d-94b5-fcefbe69dbed
σˣ = sigmax(SpinBasis(1/2))

# ╔═╡ f24f583d-bbb0-4f6d-bb49-51e270670e4c
σʸ = sigmay(SpinBasis(1/2))

# ╔═╡ 6c3f7e5f-37cd-4535-9fe4-1d630058c62a
σᶻ = sigmaz(SpinBasis(1/2))

# ╔═╡ 88e41923-4866-4e56-ba66-93f070263933
function Hstatic(N)
...
# ╔═╡ 708a59b9-ddb4-4ad0-863d-0941917cc1f1
function Hdyn(N)
	# vector of operators
	🥭 = [σᶻ; fill(id, N-1)]
...
# ╔═╡ 4bd1feb6-131c-4c70-bddb-daada43ee941
function H_t(t, gvars, N)
	Hstatic(N) + g_t(t, gvars)*Hdyn(N)
end

# ╔═╡ a225056a-5275-4035-a1ce-cc22888eddf8
...
# ╔═╡ 6179087c-4fad-470e-8cf0-7feed46c9c82
tmin = 0.5

# ╔═╡ a7586e0e-aa32-4385-8a41-51fd3bc53f1c
tmax = tau

# ╔═╡ b28a6ee8-6b87-4c8b-85d8-33ec94d16796
fidel_interval = vcat(LinRange(tmin,tmax,amt_times))

# ╔═╡ e1bf702e-93e9-4187-ab89-d9d073b352f5
eigens_0, eigsts_0 = eigenstates(dense(H_t(0, gvars, npart)), 2)

# ╔═╡ ee0e2044-f5be-4226-8d23-dfd1b6d27d75
gs1_0 = eigsts_0[1]

# ╔═╡ ed56327d-3491-4b8d-bbcc-68c4b252cc77
gs2_0 = eigsts_0[2]

# ╔═╡ 72a34aee-ddc6-427c-b652-690f93f53a07
eigenslin_0, eigstslin_0 = eigenstates(dense(H_lin_t(0, gvars_lin, npart)), 2)

# ╔═╡ 52fead29-5d2b-45bf-8178-bed5a9c971c7
gslin1_0 = eigstslin_0[1]

# ╔═╡ 70cbf865-3b78-46ba-8f63-ee464d413d0f
gslin2_0 = eigstslin_0[2]

# ╔═╡ 0a267025-7582-4273-b5ca-500186331846
function ℙₑ(state)
	round((1 + expect(tensor(fill(σᶻ, npart)...), state))/2)
end

# ╔═╡ 937de0ef-144b-4120-a954-be0bfdfc6c44
function ℙₒ(state)
	round((1 - expect(tensor(fill(σᶻ, npart)...), state))/2)
end

# ╔═╡ fbcae2aa-6d18-46d8-9c2b-886f2135d685
gse_0 = ℙₑ(gs1_0)*gs1_0 + ℙₑ(gs2_0)*gs2_0

# ╔═╡ ffe9a44e-782b-4a2e-9ee6-e410c1d254e4
gso_0 = ℙₒ(gs1_0)*gs1_0 + ℙₒ(gs2_0)*gs2_0

# ╔═╡ cc18eeba-5b9a-44d9-83c2-0c456bdaa861
gsline_0 = ℙₑ(gs1_0)*gslin1_0 + ℙₑ(gs2_0)*gslin2_0

# ╔═╡ aac7eb7a-558d-4833-ae15-cd3caacf962c
gslino_0 = ℙₒ(gs1_0)*gslin1_0 + ℙₒ(gs2_0)*gslin2_0

# ╔═╡ 102fb036-4289-48fc-a416-f3bd8287eaf0
fidele = zeros(amt_times)

# ╔═╡ e8056674-0601-4ee7-85f1-2043c95326ae
fidelo = zeros(amt_times)

# ╔═╡ 4e705e92-ad8d-46ee-84ef-0ee3fb1d1f97
fidel_line = zeros(amt_times)

# ╔═╡ 2b098cee-be7b-4c98-bca5-2e737c725bcd
fidel_lino = zeros(amt_times)

# ╔═╡ aaf9abaf-48e0-4660-a2b1-7b604a4ab7e0
...
# ╔═╡ fab0ed53-6c64-4078-968e-ba7b1fbac119
begin
    # Plot the results
    fig, ax = plt.subplots()
...
manifest_format = "2.0"
project_hash = "65819f700c2d15e644300965d4cec2defb39c264"
...
deps = ["LinearAlgebra", "SparseArrays"]
git-tree-sha1 = "7191089fc6664dfebd3a25125c7b2318fdb21c70"
...
git-tree-sha1 = "fbb3d6cac7eee202fde6e2b60f0aad8d1138a07e"
