...
for index in 1:amt_times
	...
	fidel1[index] = norm(dagger(es_t)*evol1[2][2])^2
	fidel2[index] = norm(dagger(gs_t)*evol2[2][2])^2
	fidel_lin1[index] = norm(dagger(es_lin_t)*evol_lin1[2][2])^2
...
begin
    ...
    ax.plot(fidel_interval, fidel1, color="firebrick", linestyle="-.", label="MA odd")
    ax.plot(fidel_interval, fidel_lin1, color="blue", linestyle="-.", label="LR odd")
    ax.plot(fidel_interval, fidel2, color="firebrick", label="MA even")
    ax.plot(fidel_interval, fidel_lin2, color="blue", label="LR even")
